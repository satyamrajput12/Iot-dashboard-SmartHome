...
<div class="table-card">
    <div class="p-3 border-bottom border-light border-opacity-10">
        <span style="font-size:.85rem;color:var(--text-muted);">{{ $logs->total() }} log entries</span>
    </div>
    @if($logs->isEmpty())
        <div class="text-center py-5" style="color: var(--text-muted);">No logs found for the selected filters.</div>
    @else
        ...
        <table class="table table-hover mb-0">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="font-size:0.95rem;color:var(--text-main);max-width:250px">
                        <span title="{{ $log->message }}">{{ \Illuminate\Support\Str::limit($log->message, 80) }}</span>
                    </td>
                    <td style="font-size:0.95rem">
                        @if($log->device)
                            <div class="fw-500" style="color:var(--text-main);">{{ $log->device->name }}</div>
                            <div style="font-size:0.85rem;color:var(--text-muted);">{{ $log->device->device_id }}</div>
                        @else
                            <span style="color:var(--text-muted);">—</span>
                        @endif
                    </td>
                    <td style="font-size:0.95rem">{{ $log->user->name ?? '—' }}</td>
                    <td style="font-size:0.9rem;color:var(--text-muted);font-family:monospace">{{ $log->ip_address ?? '—' }}</td>
                    <td style="font-size:0.9rem;color:var(--text-muted);white-space:nowrap">{{ $log->created_at->format('M d, H:i') }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
